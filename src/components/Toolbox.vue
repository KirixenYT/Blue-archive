  ...
  Modal.open({
    ...
    content: () => [
      h(
        'p',
        {},
        config.author === 'kirixenyt'
          ? `© ${new Date().getFullYear()} kirixenyt`
          : [`© ${new Date().getFullYear()} ${config.author}`, h('p', {}, 'Made by kirixenyt')]
      ),
      h('span', {}, 'project'),
      h('a', { href: 'https://github.com/kirixenyt', target: '_blank' }, 'Github'),
      config.ICP
        ? [
            h('br', {}, ''),
            h('a', { href: 'https://www.youtube.com/@kirixenyt', target: '_blank' }, config.ICP)
          ]
        : null
    ],
    footer: false
